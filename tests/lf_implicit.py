...
def main():
    ...
    assert sol.get_num_iterations() == approx(9, abs=2)
    assert bdy.get_lift_coef() == approx(0.1630, abs=5e-3)
    assert bdy.get_drag_coef() == approx(0.0740, abs=5e-4)
    assert bdy.get_pitch_coef() == approx(-0.0223, abs=5e-4)
